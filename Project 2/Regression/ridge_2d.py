...
2d data using l2 discounted error (ridge
regression). It takes the name of test
data file as input andproduces plots and
data to show the regression results.
-------------------------------------------
...
2022-03-03 00:52:19 -- modularised the code
...
def train(input_data, sampling_rate=200, max_degree=6):
    ...
    for deg in range(0,max_degree):
        M = (deg*deg + 3*deg + 2)//2
        lam_values = np.logspace(-20,0,40)
        for lam in lam_values:
            ...
            # ERMS(D) = SQRT (1/N SUM_{i = 1}^{N} ((t_i - f(x_i,w*)))^2 + L2-penalty)
    ...
    return wstarstar, min_rms, opt_M, opt_lam

def test_model(results, tfile):
    # io part - get test data
    wstarstar, min_rms, opt_M, opt_lam = results
    X1test = []
    X2test = []
    Ytest = []
    ...
    ax.plot3D(plots_x1,plots_x2,plots_y,color='red')
    ax.scatter3D(X1test,X2test,Ytest,color='blue')
