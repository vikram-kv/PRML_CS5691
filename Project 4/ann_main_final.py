...
if __name__ == '__main__':
    ...
    net = ANN(trainData.shape[1], dataset_type=mode).to('cuda')
    loss_criterion = T.nn.BCELoss().cuda() if mode == 'binary' else T.nn.CrossEntropyLoss().cuda()
    
    encoder = preprocessing.LabelEncoder()
    encoder.fit(trainLabels)
    encodedTrainLabels = encoder.transform(trainLabels)
    encodedTestLabels = encoder.transform(devTrueLabels)
    losses = training_loop(net, trainData, encodedTrainLabels, devData, encodedTestLabels, loss_criterion, loss_margin)
    
    print(devData.shape)
    probs = net.forward(T.Tensor(devData).cuda()).detach().cpu().numpy()
    print(probs.shape)
    predLabels = []
